...
生成から品質チェック・自動修正まで一括で実行します。

```
ALB→ECS→Aurora の3層構成図をQAループで作成して。出力先: ./architecture.drawio
```

既存の `.drawio` ファイルを自動修正する場合:

```
output.drawio を QA ループで自動修正して
```

QAループは最大3回の修正を試み、CRITICAL/ERROR = 0 になれば PASS を報告します。
...
- **`diagram-fixer`**: レビューレポートを受け取り、DrawIO XMLに最小限の差分修正を適用（R02/R03/R04/R09/R10）
